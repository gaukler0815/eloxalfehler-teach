...
test('a slingshot shot can clear l01 within its 3 shots', () => {
  let won = false;
  ...
  for (let dx = -280; dx <= -120 && !won; dx += 40) {
    for (let dy = 60; dy <= 280 && !won; dy += 40) {
      ...
      game.loadLevel(ER.levels.get('l01'));
      let guard = 6;
  ...
  assert.ok(won, 'expected at least one aim to clear l01');
});
...
test('arc kills only the rail-side enemy until the chain is completed', () => {
  const game = ER.game.create();
  game.loadLevel(ER.levels.get('l08'));
